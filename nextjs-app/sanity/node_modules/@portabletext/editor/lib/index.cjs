...
const debug$3 = plugin_eventListener.debugWithName("components:DraggableBlock"), DraggableBlock = (t0) => {
  ...
  $[11] !== blockElement || $[12] !== editor || $[13] !== element ? (t7 = (event) => {
    const isMyDragOver = plugin_eventListener.IS_DRAGGING_BLOCK_ELEMENT.get(editor);
    if (!isMyDragOver || !blockElement)
      return;
    event.preventDefault(), event.dataTransfer.dropEffect = "move", plugin_eventListener.IS_DRAGGING_ELEMENT_TARGET.set(editor, element);
    ...
    if (element === editor.children[0] || (loc < height / 2 ? plugin_eventListener.IS_DRAGGING_BLOCK_TARGET_POSITION.set(editor, "top") : plugin_eventListener.IS_DRAGGING_BLOCK_TARGET_POSITION.set(editor, "bottom")), isMyDragOver === element) {
  ...
  $[16] !== editor || $[17] !== element ? (t9 = (event_0) => {
    const targetBlock = plugin_eventListener.IS_DRAGGING_ELEMENT_TARGET.get(editor);
    if (targetBlock) {
      plugin_eventListener.IS_DRAGGING.set(editor, !1), event_0.preventDefault(), event_0.stopPropagation(), plugin_eventListener.IS_DRAGGING_ELEMENT_TARGET.delete(editor), dragGhostRef.current && (debug$3("Removing drag ghost"), document.body.removeChild(dragGhostRef.current));
      const dragPosition = plugin_eventListener.IS_DRAGGING_BLOCK_TARGET_POSITION.get(editor);
      plugin_eventListener.IS_DRAGGING_BLOCK_TARGET_POSITION.delete(editor);
  ...
  $[19] !== editor || $[20] !== element ? (t10 = (event_1) => {
    plugin_eventListener.IS_DRAGGING_BLOCK_ELEMENT.get(editor) && (debug$3("On drop (prevented)", element), event_1.preventDefault(), event_1.stopPropagation(), setIsDragOver(!1));
  }, $[19] = editor, $[20] = element, $[21] = t10) : t10 = $[21];
  const handleDrop = t10;
  let t11;
  $[22] !== editor || $[23] !== element || $[24] !== isVoid ? (t11 = (event_2) => {
    if (!isVoid) {
      plugin_eventListener.IS_DRAGGING_BLOCK_ELEMENT.delete(editor);
      return;
    }
    plugin_eventListener.IS_DRAGGING.set(editor, !0), plugin_eventListener.IS_DRAGGING_BLOCK_ELEMENT.set(editor, element), event_2.stopPropagation();
  ...
  $[26] !== blockElement || $[27] !== editor || $[28] !== handleDrag || $[29] !== isInline || $[30] !== isVoid ? (t12 = (event_3) => {
    if (!isVoid || isInline) {
      debug$3("Not dragging block"), plugin_eventListener.IS_DRAGGING_BLOCK_ELEMENT.delete(editor), plugin_eventListener.IS_DRAGGING.set(editor, !1);
      return;
    }
    if (debug$3("Drag start"), plugin_eventListener.IS_DRAGGING.set(editor, !0), event_3.dataTransfer && (event_3.dataTransfer.setData("application/portable-text", "something"), event_3.dataTransfer.effectAllowed = "move"), blockElement && blockElement instanceof HTMLElement) {
  ...
  let t13;
  $[32] !== editor || $[33] !== isDragOver ? (t13 = isDragOver && editor.children[0] === plugin_eventListener.IS_DRAGGING_ELEMENT_TARGET.get(editor), $[32] = editor, $[33] = isDragOver, $[34] = t13) : t13 = $[34];
  const isDraggingOverFirstBlock = t13;
  let t14;
  $[35] !== editor || $[36] !== isDragOver ? (t14 = isDragOver && editor.children[editor.children.length - 1] === plugin_eventListener.IS_DRAGGING_ELEMENT_TARGET.get(editor), $[35] = editor, $[36] = isDragOver, $[37] = t14) : t14 = $[37];
...
plugin_eventListener.debugWithName("components:Element");
const EMPTY_ANNOTATIONS = [], inlineBlockStyle = {
  ...
  const block_0 = plugin_eventListener.fromSlateValue([element], schemaTypes.block.name, plugin_eventListener.KEY_TO_VALUE_ELEMENT.get(editor))[0];
...
const debug$2 = plugin_eventListener.debugWithName("components:Leaf"), EMPTY_MARKS = [], Leaf = (props) => {
  const {
    editorActor,
    attributes,
    children,
    ...
    renderDecorator,
    renderAnnotation
  } = props, spanRef = React.useRef(null), portableTextEditor = plugin_eventListener.usePortableTextEditor(), blockSelected = slateReact.useSelected(), [focused, setFocused] = React.useState(!1), [selected, setSelected] = React.useState(!1), block = children.props.parent, path = React.useMemo(() => block ? [{
    _key: block?._key
  }, "children", {
    _key: leaf._key
  ...
  React.useEffect(() => {
    ...
    const sel = plugin_eventListener.PortableTextEditor.getSelection(portableTextEditor);
    sel && isEqual__default.default(sel.focus.path, path) && plugin_eventListener.PortableTextEditor.isCollapsedSelection(portableTextEditor) && React.startTransition(() => {
      setFocused(!0);
    });
  ...
  React.useEffect(() => {
    ...
    const onBlur = editorActor.on("blur", () => {
      ...
      const sel_0 = plugin_eventListener.PortableTextEditor.getSelection(portableTextEditor);
      sel_0 && isEqual__default.default(sel_0.focus.path, path) && plugin_eventListener.PortableTextEditor.isCollapsedSelection(portableTextEditor) && setFocused(!0), setSelectedFromRange();
    }), onSelection = editorActor.on("selection", (event) => {
      event.selection && isEqual__default.default(event.selection.focus.path, path) && plugin_eventListener.PortableTextEditor.isCollapsedSelection(portableTextEditor) ? setFocused(!0) : setFocused(!1), setSelectedFromRange();
    });
    return () => {
...
const debug$1 = plugin_eventListener.debugWithName("plugin:withHotKeys");
...
const debug = plugin_eventListener.debugWithName("component:Editable"), PLACEHOLDER_STYLE = {
  position: "absolute",
  userSelect: "none",
  pointerEvents: "none",
  left: 0,
  ...
  const {
    ...
    spellCheck,
    ...restProps
  } = props, portableTextEditor = plugin_eventListener.usePortableTextEditor(), ref = React.useRef(null), [editableElement, setEditableElement] = React.useState(null), [hasInvalidValue, setHasInvalidValue] = React.useState(!1), [rangeDecorationState, setRangeDecorationsState] = React.useState([]);
  React.useImperativeHandle(forwardedRef, () => ref.current);
  const rangeDecorationsRef = React.useRef(rangeDecorations), editorActor = React.useContext(plugin_eventListener.EditorActorContext), readOnly = react.useSelector(editorActor, (s) => s.matches({
    "edit mode": "read only"
  ...
  const renderElement = React.useCallback((eProps) => /* @__PURE__ */ jsxRuntime.jsx(Element, { ...eProps, readOnly, renderBlock, renderChild, renderListItem, renderStyle, schemaTypes, spellCheck }), [schemaTypes, spellCheck, readOnly, renderBlock, renderChild, renderListItem, renderStyle]), renderLeaf = React.useCallback((lProps) => {
    ...
    if (propsSelection) {
      ...
      const normalizedSelection = normalizeSelection(propsSelection, plugin_eventListener.fromSlateValue(slateEditor.children, blockTypeName));
      if (normalizedSelection !== null) {
        debug(`Normalized selection from props ${JSON.stringify(normalizedSelection)}`);
        const slateRange = plugin_eventListener.toSlateRange(normalizedSelection, slateEditor);
    ...
    if (rangeDecorations && rangeDecorations.length > 0) {
      ...
      if (rangeDecorations.forEach((rangeDecorationItem) => {
        const slateRange_0 = plugin_eventListener.toSlateRange(rangeDecorationItem.selection, slateEditor);
        ...
        let newRange;
        if (operation && (newRange = plugin_eventListener.moveRangeByOperation(slateRange_0, operation), newRange && newRange !== slateRange_0 || newRange === null && slateRange_0)) {
          const value = plugin_eventListener.PortableTextEditor.getValue(portableTextEditor), newRangeSelection = plugin_eventListener.toPortableTextRange(value, newRange, schemaTypes);
          rangeDecorationItem.onMoved && rangeDecorationItem.onMoved({
  ...
  const handleCopy = React.useCallback((event) => {
    ...
    const value_0 = plugin_eventListener.PortableTextEditor.getValue(portableTextEditor), path = plugin_eventListener.toPortableTextRange(value_0, slateEditor.selection, schemaTypes)?.focus.path || [], onPasteResult = onPaste?.({
      event: event_0,
      value: value_0,
      path,
      schemaTypes
    ...
    onPasteResult || !slateEditor.selection ? (event_0.preventDefault(), editorActor.send({
      ...
      debug("Custom paste function from client resolved", result_0), !result_0 || !result_0.insert ? (debug("No result from custom paste handler, pasting normally"), slateEditor.insertData(event_0.clipboardData)) : result_0.insert ? slateEditor.insertFragment(plugin_eventListener.toSlateValue(result_0.insert, {
        schemaTypes
    ...
    if (onFocus && onFocus(event_1), !event_1.isDefaultPrevented()) {
      const selection = plugin_eventListener.PortableTextEditor.getSelection(portableTextEditor);
      selection === null && (slate.Transforms.select(slateEditor, slate.Editor.start(slateEditor, [])), slateEditor.onChange()), editorActor.send({
        type: "notify.focused",
        event: event_1
      });
      const newSelection = plugin_eventListener.PortableTextEditor.getSelection(portableTextEditor);
  ...
  const handleKeyDown = React.useCallback((event_5) => {
    ...
    if (plugin_eventListener.isEqualToEmptyEditor(slateEditor.children, schemaTypes))
      return [{
        anchor: {
          path: [0, 0],
          offset: 0
...
exports.EditorEventListener = plugin_eventListener.EventListenerPlugin;
exports.EditorProvider = plugin_eventListener.EditorProvider;
exports.PortableTextEditor = plugin_eventListener.PortableTextEditor;
exports.defineSchema = plugin_eventListener.defineSchema;
exports.keyGenerator = plugin_eventListener.defaultKeyGenerator;
exports.useEditor = plugin_eventListener.useEditor;
exports.useEditorSelector = plugin_eventListener.useEditorSelector;
exports.usePortableTextEditor = plugin_eventListener.usePortableTextEditor;
exports.usePortableTextEditorSelection = plugin_eventListener.usePortableTextEditorSelection;
