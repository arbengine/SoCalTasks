...
export default function PageBuilder({ page }: PageBuilderPageProps) {
  const [pageBuilderSections, optimisticUpdateFn] = useOptimistic(
    page?.pageBuilder || [],
    (currentSections: PageBuilderSection[], update: SanityUpdate) => {
      // If the edit was to a different document, ignore it
      if (update._id !== page?._id) {
        return currentSections;
      }

      // If there are sections in the updated document, use them
      if (update.pageBuilder) {
        return update.pageBuilder.map((section: PageBuilderSection) =>
